...
function restoreStoryboard(state, container, sessionId) {
    ...
    if (["planning_round_1"].includes(state.status)) {
        ...
        if (state.partialStreams) {
            Object.entries(state.partialStreams).forEach(([agentKey, chunk]) => {
                if (!state.planningBoard.round1Proposals[agentKey]) {
                    const agentName = AGENTS[agentKey]?.name || agentKey;
                    const div = document.createElement('div');
                    div.className = 'agent-contribution';
                    div.innerHTML = `<div class="agent-role">${agentName}</div><div class="agent-text">${marked.parse(chunk || '')}<div class="typing-indicator"><span></span><span></span><span></span></div></div>`;
                    planContent.appendChild(div);
                }
            });
        }
        ...
        if (state.status === "planning_round_2" && state.partialStreams) {
            Object.entries(state.partialStreams).forEach(([agentKey, chunk]) => {
                if (!state.planningBoard.finalAssignments[agentKey]) {
                    const agentName = AGENTS[agentKey]?.name || agentKey;
                    const div = document.createElement('div');
                    div.className = 'agent-contribution';
                    div.innerHTML = `<div class="agent-role">${agentName}</div><div class="agent-text">${marked.parse(chunk || '')}<div class="typing-indicator"><span></span><span></span><span></span></div></div>`;
    ...
    if (findingsDisplay === "flex") {
        ...
        if (state.status === "retrieval" && state.partialStreams) {
            Object.entries(state.partialStreams).forEach(([agentName, chunk]) => {
                if (!state.findingsBoard.find(f => f.agent === agentName)) {
                    const cardDiv = document.createElement('div');
                    cardDiv.className = 'finding-card';
                    cardDiv.innerHTML = `<div class="finding-source">${agentName}</div><div class="finding-text">${marked.parse(chunk || '')}<div class="typing-indicator"><span></span><span></span><span></span></div></div>`;
                    findingsContent.appendChild(cardDiv);
                }
            });
        }
    ...
    if (cpDisplay === "flex") {
        ...
        } else if (state.status === "checkpoint" && state.partialStreams && state.partialStreams['evaluator']) {
            const cpDiv = document.createElement('div');
            cpDiv.className = 'agent-contribution';
            cpDiv.innerHTML = `<div class="agent-role">Evaluator</div><div class="agent-text">${marked.parse(state.partialStreams['evaluator'])}<div class="typing-indicator"><span></span><span></span><span></span></div></div>`;
            cpContent.appendChild(cpDiv);
        }
    }
}
...
if "function throttledSaveSession" not in content:
    content = content.replace("function saveCurrentSession() {", helpers + "\nfunction saveCurrentSession() {")

# 2. Modify renderHistory
old_render = """function renderHistory() {
...
new_render = """function renderHistory() {
    ...
    if (mState && mState.status === "synthesis" && mState.partialStreams && mState.partialStreams['synthesis']) {
        const lastMsg = mState.partialStreams['synthesis'];
        appendMessage('assistant', marked.parse(lastMsg) + '<div class="typing-indicator"><span></span><span></span><span></span></div>', 'Hermes Orchestrator (Synthesis)', true, container);
    }
...
# 3. Modify loadSession
old_load = """        if (!processingSessions[id]) {
...
new_load = """        globalHistory = session.history;
        if (session.missionState) globalMissionStates[id] = session.missionState;
        renderHistory();
...
# 4. Extract executeMission and replace submit handler
submit_start = "chatForm.addEventListener('submit', async (e) => {"
submit_end_pattern = r"        processingSessions\[thisSessionId\] = false;\s*if\s*\(currentSessionId === thisSessionId\)\s*\{\s*sendBtn\.disabled = false;\s*messageInput\.focus\(\);\s*\}\s*\}\);"
match = re.search(submit_end_pattern, content)
if match:
    submit_end_index = match.end()
    submit_start_index = content.find(submit_start)
    
    old_submit_block = content[submit_start_index:submit_end_index]
...
"""
    content = content[:submit_start_index] + new_logic + content[submit_end_index:]
